...
@Repository
public class NmfV2Recommender implements IRecommender {
    ...
    public List<VideoDoc> recommend(RecommendContext recommendContext) {
        ...
        taggedMetricRegistry.meter("nmfv2-video.request.qps").mark();
        Timer.Context time = taggedMetricRegistry.timer("nmfv2.request.latency").time();
        ResDoc resDoc = null;
        ...
        if (resDoc != null && resDoc.getStatus() == 0 && resDoc.getData() != null) {
            taggedMetricRegistry.histogram("nmfv2.recall.failrate").update(0);
            return resDoc.getData();
        } else {
            taggedMetricRegistry.histogram("nmfv2.recall.failrate").update(100);
