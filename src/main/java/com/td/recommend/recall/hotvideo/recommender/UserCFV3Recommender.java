...
public class UserCFV3Recommender implements IRecommender{
    ...
    public List<VideoDoc> recommend(RecommendContext recommendContext) {
        taggedMetricRegistry.meter("usercfv3.request.qps").mark();
        Timer.Context time = taggedMetricRegistry.timer("usercfv3.request.latency").time();
        ...
        time.stop();
        if (docs.size() > 0) {
            taggedMetricRegistry.histogram("usercfv3.recall.failrate").update(0);
        } else {
            taggedMetricRegistry.histogram("usercfv3.recall.failrate").update(100);
        }
        return docs;
    }
    ...
    private List<String> getAllClicks(List<VideoDoc> resDocs) {
        List<String> videos;
        Timer.Context time = taggedMetricRegistry.timer("usercfv3.getclicks.latency").time();
        ...
        try {
            ...
            LOG.error("usercfv3.getclicks failed!", e);
        }

        time.stop();

        if(videos.size()>0) {
            taggedMetricRegistry.histogram("usercfv3.sim.clicks.count").update(videos.size() / userNum);
        }
        return videos;
