...
public class NmfV3Recommender implements IRecommender{
    ...
    static {
        ...
        url = conf.getString("nmfv3.u2i.url");
    }
    ...
    public List<VideoDoc> recommend(RecommendContext recommendContext) {
        ...
        taggedMetricRegistry.meter("nmfv3-video.request.qps").mark();
        Timer.Context time = taggedMetricRegistry.timer("nmfv3.request.latency").time();
        ResDoc resDoc = null;
        ...
        try {
            ...
            String requestUrl = url + "key=" + key + "&num=" + recommendContext.getNum() + "&appid=" + appid;
        ...
        if (resDoc != null && resDoc.getStatus() == 0 && resDoc.getData() != null) {
            taggedMetricRegistry.histogram("nmfv3.recall.failrate").update(0);
            return resDoc.getData();
        } else {
            taggedMetricRegistry.histogram("nmfv3.recall.failrate").update(100);
